...
usersApp.service('popups', function () {
    ...
    self.fillProfile = function (progileBtn) {
        var editForm = document.getElementById('EditForm'),
		input = editForm.getElementsByTagName("input"),
		id = progileBtn.parentNode.id;
		$.post('http://localhost:4001/user/' + id, $('#add').serialize(), function(response) {
...
usersApp.service('control', ['popups', function (popups) {
    ...
    self.submitAddForm = function(isValid){
        if (isValid){
            $.post('http://localhost:4001/add', $('#add').serialize(), function(response) {
    ...
    self.submitEditForm = function(isValid) {
        if (isValid){
            ...
            $.post('http://localhost:4001/update/' + id, $('#userEdit').serialize(), function(response) {
                if (response.error !== undefined) {
...
usersApp.controller('usersController',['$scope', '$http', 'popups', 'filters', 'control', function($scope, $http, popups, filters, control) {
    $scope.openProfile = function(progileBtn){
        popups.show('EditForm');
        popups.fillProfile(progileBtn);
    }
    ...
    $.get('http://localhost:4001/users').then(function(response) { 
        $scope.users = {
            info: response
        };
    });
