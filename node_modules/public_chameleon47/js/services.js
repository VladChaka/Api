var moduleService = angular.module("usersApp");

moduleService.service('popups', function() {
    var self = this;
...
moduleService.service('filters', function() {
    var self = this;
...
moduleService.service('control', ['popups', 'userRemoteService', function(popups, userRemoteService) {
    var self = this;

    self.clearForm = function(form) {
    ...
    self.deleteUser = function(deleteBtn) {
        ...
        userRemoteService.delete.delete({ userId: id });
        deleteBtn.parentNode.parentNode.style.display = "none";
	}
	
	self.authUser = function(authBtn) {
        //if (isValid) {
            let input = authBtn.parentNode.getElementsByTagName("input"),
                dataJson = {
                    username: input[0].value,
                    password: input[1].value
				};
            userRemoteService.auth.save(dataJson, function(response) {
    ...
    self.submitAddForm = function(isValid) {
        if (isValid) {
            ...
            userRemoteService.create.save(dataJson, function(response) {
                if (response.error !== undefined) {
    ...
    self.submitEditForm = function(isValid) {
        if (isValid) {
            ...
            userRemoteService.update.save(dataJson, function(response) {
                if (response.error !== undefined) {
                    if (response.error === "This email duplicate") {
                        document.getElementsByClassName('edit-email-used')[0].style.display = "block";
                    }
    ...
    self.fillProfile = function(profileBtn) {
        ...
        var input = editForm.getElementsByTagName("input");
        userRemoteService.getById.get({ userId: id }).$promise.then(function(user) {
